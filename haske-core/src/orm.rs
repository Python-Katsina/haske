...
#[pyfunction]
pub fn prepare_query(py: Python<'_>, sql: &str, params: Bound<'_, PyDict>) -> PyResult<(String, Vec<PyObject>)> {
    ...
    for name in param_names {
        match params.get_item(name.as_str()) {
            Ok(Some(val)) => vals.push(val.into_py(py)),
            Ok(None) => vals.push(py.None().into_py(py)),
            Err(_) => return Err(pyo3::exceptions::PyKeyError::new_err(format!("parameter '{}' not found", name))),
        }
    }
...
#[pyfunction]
pub fn prepare_queries(py: Python<'_>, queries: Vec<String>, params: Vec<Bound<'_, PyDict>>) -> PyResult<Vec<(String, Vec<PyObject>)>> {
