...
pub struct HaskeApp {
    ...
    route_cache: Arc<RwLock<HashMap<(String, String), (Py<PyAny>, PyObject)>>>,
}
...
impl HaskeApp {
    ...
    pub fn add_route(&self, method: &str, path_regex: &str, handler: Py<PyAny>) -> PyResult<()> {
        ...
        let param_names: Vec<String> = regex.capture_names().flatten().map(|s| s.to_string()).collect();
    ...
    pub fn match_request<'py>(&self, py: Python<'py>, method: &str, path: &str) -> PyResult<Option<(PyObject, PyObject)>> {
        // Check cache first
        let cache_key = (method.to_uppercase(), path.to_string());
        {
            let cache = self.route_cache.read();
            if let Some(result) = cache.get(&cache_key) {
                return Ok(Some((result.0.clone().into_py(py), result.1.clone())));
            }
        }
        
        let routes = self.routes.read();
        for r in routes.iter() {
            if r.method != "ANY" && r.method != method.to_uppercase() { continue; }
            if let Some(caps) = r.regex.captures(path) {
                ...
                let handler = r.handler.clone().into_py(py);
                let dict_obj: PyObject = dict.into();  // Added type annotation
                
                // Cache the result
                let mut cache = self.route_cache.write();
                cache.insert(cache_key, (r.handler.clone(), dict_obj.clone()));
